"""render/build_slice3.py — Phase 5 slice 3 부분 tree 렌더 빌더 (TIMEZONE family).

★ GAP-25 결정: Phase 8 렌더 정본 = inline(spike_interaction.html 방식). slice1의 CDN 답습 금지.
build_slice2.py와 동일하게 spike_interaction.html의 inline 라이브러리 3블록(cytoscape 3.30.2 +
dagre 0.8.5 + cytoscape-dagre 2.5.0)을 추출해 오프라인 단일 HTML(render/slice3_timezone.html)을
...
표시(★ slice 3 = TIME mess 블록 마무리): TIMEZONE 쌍(c0312 DETECT → c0313 NORMALIZE)을 강조하고,
형제 TIME mess 쌍(FORMAT c0310/c0311, ANCHOR c0314/c0315)을 context로 함께 그려 세 쌍이 모두
backbone(axis 평가 진입) 노드로 수렴하는 모습을 보인다. TIMEZONE은 can_route_to_q=[] → Q-terminal
노드/패널 없음(D-S4 무기여). c0313 postcond는 vacuous-default(GAP-27) — 패널에 명시 표기.
"""
...
<title>pmx-dt · Phase 5 slice 3 — TIMEZONE family 부분 tree</title>
...
<body>
<header>
  <h1>Phase 5 · slice 3 — TIMEZONE family 부분 decision tree</h1>
  <div class="sub">TIME mess 블록 마무리: <b>c0312 DETECT → c0313 NORMALIZE</b>(주황=이번 슬라이스) + 형제 FORMAT·ANCHOR 쌍 → backbone(axis 평가) 수렴. TIMEZONE은 Q-code 없음(can_route_to_q=[]). 라이브러리=inline(offline, GAP-25).</div>
...
<div id="legend">
  ...
  <span class="lg"><span class="sw" style="background:var(--slice);border-color:#b35a16"></span>이번 슬라이스(TIMEZONE)</span>
...
var LS_KEY="pmx_dt_slice3_state";
...
var CUNITS={
  c0312:{c_id:"c0312",c_name_ko:"시간대 감지",srp_intent:"DETECT TIMEZONE",kind:"detect",cost:1,requires_detection_by:null,layer:"L-4->L-5",slice:true,
    postcond:"isinstance(meta.get('tz_issues'), dict)",can_route_to_q:[],
    vv:{target_columns:["time_value"],criterion_predicate_ko:"시간대 불일치 여부가 파악됨",pass_route_to:"c0313",fail_route_to:null},ref:"universe_sm §6 TIMEZONE"},
  c0313:{c_id:"c0313",c_name_ko:"시간대 정규화",srp_intent:"NORMALIZE TIMEZONE",kind:"transform",cost:2,requires_detection_by:"c0312",layer:"L-4->L-5",slice:true,
    postcond:"meta.get('tz_normalized', True)",can_route_to_q:[],vv:null,
    note:"★ postcond는 default=True라 no-op도 vacuously 통과(GAP-27). spec frozen — 구현/trap이 실제 정규화·flag 명시 set·missing-detection(success=False)을 강제(override 아님).",ref:"universe_sm §6 TIMEZONE"},
...
/* ---- elements: 세 TIME mess 쌍 → backbone(axis 평가) 수렴 ---- */
...
function layoutOpts(){ return {name:(dagreOK?"dagre":"breadthfirst"),rankDir:"LR",directed:true,fit:true,padding:30,animate:false,nodeSep:40,rankSep:96,spacingFactor:1.0}; }
...
function renderCPanel(id){
  ...
  var h='<div class="sect"><h3>(A) Identity'+(c.slice?' <span class="pill slice">slice 3</span>':'')+'</h3><div class="body">';
...
function renderBackbonePanel(){
  ...
  h+='<div class="kv">N2 수렴: TIMEZONE·TIME_FORMAT·TIME_ANCHOR 세 mess 쌍이 이 노드 앞에서 종료(D-S3: mess가 backbone 앞단).</div>';
  h+='<div class="kv">N3 slice 3 기여 strand: <b>532</b> (c0312/c0313 freq). TIMEZONE은 can_route_to_q=[] → 고립 Q-terminal 무기여(D-S4).</div>';
  h+='</div></div>';
  return h;
}
...
HTML = PAGE_HEAD + "\n" + LIBS + "\n" + APP_SCRIPT
OUT = ROOT / "render" / "slice3_timezone.html"
